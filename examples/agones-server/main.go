...
func main() {
	...
	passthrough := flag.Bool(
		"passthrough",
		false,
		"Get listening port from the SDK, rather than use the 'port' value",
	)
	readyOnStart := flag.Bool("ready", true, "Mark this GameServer as Ready on startup")
	shutdownDelayMin := flag.Int(
		"automaticShutdownDelayMin",
		0,
		"[Deprecated] If greater than zero, automatically shut down the server this many minutes after the server becomes allocated (please use automaticShutdownDelaySec instead)",
	)
	shutdownDelaySec := flag.Int(
		"automaticShutdownDelaySec",
		0,
		"If greater than zero, automatically shut down the server this many seconds after the server becomes allocated (cannot be used if automaticShutdownDelayMin is set)",
	)
	readyDelaySec := flag.Int(
		"readyDelaySec",
		0,
		"If greater than zero, wait this many seconds each time before marking the game server as ready",
	)
	readyIterations := flag.Int(
		"readyIterations",
		0,
		"If greater than zero, return to a ready state this number of times before shutting down",
	)
	gracefulTerminationDelaySec := flag.Int(
		"gracefulTerminationDelaySec",
		0,
		"Delay after we've been asked to terminate (by SIGKILL or automaticShutdownDelaySec)",
	)
...
) (response string, addACK bool, responseError error) {
...
func incrementCounter(s *sdk.SDK, counterName string, amount string) (string, error) {
	...
	if err != nil {
		return "false", fmt.Errorf(
			"could not increment Counter %s by unparseable amount %s: %s",
			counterName,
			amount,
			err,
		)
	...
	if err != nil {
		log.Printf(
			"Error incrementing Counter %s Count by amount %d: %s",
			counterName,
			amountInt,
			err,
		)
...
func decrementCounter(s *sdk.SDK, counterName string, amount string) (string, error) {
	...
	if err != nil {
		return "false", fmt.Errorf(
			"could not decrement Counter %s by unparseable amount %s: %s",
			counterName,
			amount,
			err,
		)
	...
	if err != nil {
		log.Printf(
			"Error decrementing Counter %s Count by amount %d: %s",
			counterName,
			amountInt,
			err,
		)
...
func setCounterCount(s *sdk.SDK, counterName string, amount string) (string, error) {
	...
	if err != nil {
		return "false", fmt.Errorf(
			"could not set Counter %s to unparseable amount %s: %s",
			counterName,
			amount,
			err,
		)
...
func setCounterCapacity(s *sdk.SDK, counterName string, amount string) (string, error) {
	...
	if err != nil {
		log.Printf(
			"Error setting Counter %s Capacity to amount %d: %s",
			counterName,
			amountInt,
			err,
		)
...
func setListCapacity(s *sdk.SDK, listName string, amount string) (string, error) {
	...
	if err != nil {
		return "false", fmt.Errorf(
			"could not set List %s to unparseable amount %s: %s",
			listName,
			amount,
			err,
		)
